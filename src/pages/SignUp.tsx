...
const SignUpPage: React.FC = () => {
  ...
  const [number, setNumber] = useState('1'); // New state for student number
  ...
  const numbers = Array.from({ length: 30 }, (_, i) => String(i + 1)); // New array for student numbers
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Assign common fields for both user types
      Object.assign(userData, {
        school,
        grade: parseInt(grade),
        class: parseInt(classNum),
      ...
      if (userType === 'student') {
        Object.assign(userData, {
          number: parseInt(number), // Add student number
          avatarConfig: {},
          warnCount: 0,
        });
      }
      ...
      <Card className="w-full max-w-lg rounded-2xl shadow-xl border border-slate-200 bg-white">
        ...
        <CardContent className="p-8 pt-0">
          ...
          <form onSubmit={handleSignUp} className="grid grid-cols-1 gap-6">
            <Tabs value={userType} onValueChange={(value) => setUserType(value as 'student' | 'teacher')} className="w-full mb-6">
            ...
            {/* Common dropdowns for both student and teacher */}
            <div className="col-span-1">
            ...
            {userType === 'student' && (
              <div className="col-span-1">
                <Label className="text-slate-700">번호</Label>
                <Listbox value={number} onChange={setNumber}>
                  <div className="relative mt-1">
                    <Listbox.Button className="relative w-full cursor-default rounded-lg bg-slate-100 py-3 pl-4 pr-10 text-left shadow-sm border-0 focus:ring-2 focus:ring-blue-300">
                      <span className="block truncate text-slate-700">{number}</span>
                      <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
                        ▼
                      </span>
                    </Listbox.Button>
            ...
            <div className="col-span-1 mt-4">
              <input type="checkbox" id="privacyConsent" required className="mr-2" />
              <Label htmlFor="privacyConsent" className="text-sm text-slate-700 leading-relaxed">
                <span className="font-semibold">(필수) 개인정보 수집 및 이용 동의</span><br />
                완도 SW교육체험센터는 원활한 서비스 제공 및 이용자 식별(교사의 학급 학생 확인 등)을 위해 이메일, 아이디, 학교, 학년, 반 등의 정보를 수집합니다. 수집된 개인정보는 회원 탈퇴 시까지 안전하게 보관되며, 동의를 거부할 경우 회원가입이 제한될 수 있습니다.
              </Label>
            </div>
